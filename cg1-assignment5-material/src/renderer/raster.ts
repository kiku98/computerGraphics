...
export class Rasterizer {
  ...
  render(s: Scene): Array<Vec4> {
    ...
    for (let m = 0; m < s.meshes.length; m++) {
      ...
      for (let i = 0; i < length; i++) {
        mesh.faces[i].primitives((v1: Vert, v2: Vert, v3: Vert): boolean => {
          this.drawTriangle(
            frameBuf,
            depthBuf,
            uniforms,
  ...
  vertexShader(v: Vert, uniforms: Map<string, Mat4>): Vert {
    ...
    const pos = v.position
      ...
      .apply(projMatrix)
      .apply(vpMatrix);
    return new Vert(pos.scale(1 / pos.w), v.normal, v.uv);
  }
  /**
  ...
  drawTriangle(
    ...
    for (let x = xmin; x < xmax; x++) {
      for (let y = ymin; y < ymax; y++) {
        ...
        if (
          !this.isInsideTriangleOld(p, t1.position, t2.position, t3.position)
        ) {
          continue;
        }
   ...
   */
  isInsideTriangleOld(p: Vec4, v1: Vec4, v2: Vec4, v3: Vec4): boolean {
   ...
   * @returns true if p is inside triangle v1v2v3
   */
  isInsideTriangleNew(w1: number, w2: number, w3: number): boolean {
