...
btn.textContent = 'Hello Computer Graphics 1 and TypeScript';
...
function sayHello(msg: string) {
  // Print a console log
  console.log('Hello cg1 from console!');
  if (msg.length !== 0) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (msg.length !== 0) {
      p.textContent = `Hello LMU Munich CG1! Your input is: ${msg}`;
    } else {
      p.textContent = 'Hello LMU Munich CG1! But you did not say anything.';
    }
