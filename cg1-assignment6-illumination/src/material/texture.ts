...
export class Texture {
  ...
  queryTrilinear(h: number, l: number, t: number, u: number, v: number): Vec4 {
    ...
    const L1 = this.queryBilinear(
      h,
      (u * this.size) / Math.pow(2, h),
      (v * this.size) / Math.pow(2, h)
    );
    const L2 = this.queryBilinear(
      l,
      (u * this.size) / Math.pow(2, l),
      (v * this.size) / Math.pow(2, l)
    );
    return LerpV(L1, L2, t);
  }
  /**
