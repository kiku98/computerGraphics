...
class Rasterizer {
     ...
     */
    constructor(width, height, msaa = 1) {
    ...
    render(s) {
        ...
        for (let m = 0; m < s.meshes.length; m++) {
            ...
            for (let i = 0; i < length; i++) {
                this.drawTriangle(frameBuf, depthBuf, uniforms, mesh.faces[i].verts[0], mesh.faces[i].verts[1], mesh.faces[i].verts[2], mesh.material);
            }
        }
    ...
    vertexShader(v, uniforms) {
        ...
        const normalMatrix = uniforms.get('normalMatrix');
        const pos = v.position
            .apply(modelMatrix)
            .apply(viewMatrix)
            .apply(projMatrix)
            .apply(vpMatrix);
        return new mesh_1.Vert(pos.scale(1 / pos.w), v.normal.apply(normalMatrix), v.uv);
    }
    /**
    ...
    drawTriangle(frameBuf, depthBuf, uniforms, v1, v2, v3, mat) {
        ...
        let t2Z = 1;
        let t3Z = 1;
        if (this.context.camera instanceof camera_1.PerspectiveCamera) {
            t1Z = 1 / t1.position.z;
        ...
        for (let x = xmin; x < xmax; x++) {
            for (let y = ymin; y < ymax; y++) {
                ...
                const normal = new vec4_1.Vec4(this.barycentricInterpolation(bc, t1.normal.x, t2.normal.x, t3.normal.x), this.barycentricInterpolation(bc, t1.normal.y, t2.normal.y, t3.normal.y), this.barycentricInterpolation(bc, t1.normal.z, t2.normal.z, t3.normal.z), 0).unit();
                ...
                const pos = new vec4_1.Vec4(px, py, pz, 1);
                // draw triangle wireframe using a different color.
                const blinnPhong = this.blinnPhong(pos, normal, mat.options, this.context.light, this.context.camera);
                // We completed the color, let's draw the pixel!
                color = (0, interpolate_1.clampV)(color.scale(blinnPhong));
     ...
     */
    blinnPhong(x, normal, matopts, light, camera) {
